...
import com.gys.play.databinding.ItemNovelStyle8Binding
...
object BtVideoStyle8 {

    fun getType(): Int {
        return Config.HOME_NOVEL_STYLE_8
    }
    ...
    fun getItemViewDelegate(activity: FragmentActivity): NovelStyleAdapter.NewUserReadItemViewDelegate {
        return object : NovelStyleAdapter.NewUserReadItemViewDelegate(
            ...
            override fun convert(holder: ViewHolder?, item: BtVideoInfo?, position: Int) {
                holder?.let {
                    val binding: ItemNovelStyle8Binding = it.getViewDataBinding()
                    ...
                    if (item == null) {
                        ...
                        if (item.list!!.size > 1) {
                            binding.listRv.models = item.list!!.subList(0, 1)
                            binding.listRv1.models = item.list!!.subList(1, item.list!!.size)
                        } else {
