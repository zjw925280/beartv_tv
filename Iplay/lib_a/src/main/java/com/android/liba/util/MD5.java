...
public class MD5 {
    private static String TAG = MD5.class.getSimpleName();
    ...
    static {
        try {
            ...
            UIHelper.showLog(TAG, "获取MD5信息摘要失败" + e);
        }
    }
    ...
    public static String string2MD5(String inStr) {
        if (sDigest == null) {
            UIHelper.showLog(TAG, "MD5信息摘要初始化失败");
            return null;
        } else if (TextUtils.isEmpty(inStr)) {
            UIHelper.showLog(TAG, "参数strSource不能为空");
            return null;
        }
    ...
    public static String encrypt(final String strSource) {
        ...
        try {
            ...
            UIHelper.showLog(TAG, "加密模块暂不支持此字符集合" + e);
        }
        return null;
    }
